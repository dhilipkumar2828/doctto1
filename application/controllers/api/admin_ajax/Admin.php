...
function get_cities()
    ...
    function validateEmail()
    {
         ...
         $qry = $this->db->query("select * from vendor_shop where email='".$email."'");
         if($qry->num_rows()>0)
         {
            echo '@success'; die;
         }
    ...
    {
         $mobile = $this->input->get_post('phone');  
         $qry = $this->db->query("select * from vendor_shop where mobile='".$mobile."'");
         if($qry->num_rows()>0)
         {
            echo '@success'; die;
         }
    ...
    function validatePincode()
    {
         ...
         $qry = $this->db->query("select * from vendor_shop where vendor_pincodes='".$pincode_id."'");
         if($qry->num_rows()>0)
         {
            echo '@success'; die;
         }
    ...
    function get_city_locations() {
        ...
        if (count($locations_res) > 0) {
            ...
            foreach ($locations_res as $loc) {
                ?>
                <option value="<?php echo $loc->id; ?>"><?php echo $loc->pincode; ?></option>
                <?php
            }
            die();
        } else {
