...
class PhonePeSDKClient_Simple {
    ...
    public function initiatePayment($merchantOrderId, $amount, $redirectUrl = null, $additionalParams = array()) {
        // Use production PhonePe credentials for doctor subscriptions
        $merchant_id = defined('PHONEPE_MERCHANT_ID') ? PHONEPE_MERCHANT_ID : 'M1Y5YWMA86HR';
        $salt_key = defined('PHONEPE_SALT_KEY') ? PHONEPE_SALT_KEY : '168028f5-f3cf-40e3-a320-120926e1dcfb';
        $key_index = defined('PHONEPE_SALT_INDEX') ? PHONEPE_SALT_INDEX : 1;
        ...
        // Create PhonePe API request - Use production endpoint
        $api_url = (defined('PHONEPE_BASE_URL') ? PHONEPE_BASE_URL : 'https://api.phonepe.com/apis/hermes') . '/pg/v1/pay';
    ...
    public function getOrderStatus($merchantOrderId, $details = true) {
        $merchant_id = defined('PHONEPE_MERCHANT_ID') ? PHONEPE_MERCHANT_ID : 'M1Y5YWMA86HR';
        $api_url = (defined('PHONEPE_BASE_URL') ? PHONEPE_BASE_URL : 'https://api.phonepe.com/apis/hermes') . '/pg/v1/status/' . $merchant_id . '/' . $merchantOrderId;
