...
class Doctors_banners_model extends CI_Model {
    ...
    {
        $qry = $this->db->query("select * from doctor_banners");
    ...
    function add($data) {
        ...
        $ins = $this->db->insert("doctor_banners",$array);
  ...
  function updateData($title,$appimage,$banner_type,$category_id,$doctor_id)
    {
        ...
        $where = array('id'=>$id);
        $ins = $this->db->update("doctor_banners",$array,$where);
        if($ins)
        {
            return TRUE;
        }
