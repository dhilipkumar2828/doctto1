...
class Vendor extends CI_Model {
    ...
    function verify_OTP($user_id,$otp,$type)
    {
        if($type=='doctor')
        {
                $qry = $this->db->query("select * from doctors where id='".$user_id."' and otp='".$otp."'");
                 if($qry->num_rows()>0)
                 {
                    ...
                    {
                      $row = $qry->row();
                      $res = array('status' =>TRUE,'message'=>"Your Registration under approval, once approved you can login",'user_id'=>$row->id,'phone'=>$row->mobile_number);
                       return $res;  
                    }
                 }   
                 else
    ...
    function checkLogin($pincode,$username,$password,$token)
    {
                ...
                if($chk->num_rows()>0)
                {
                     ...
                     $pincode_chk = $this->db->query("SELECT vendor_shop.* FROM vendor_shop INNER JOIN pincodes ON vendor_shop.vendor_pincodes=pincodes.id WHERE pincodes.pincode='".$pincode."' and (vendor_shop.email='".$username."' or vendor_shop.mobile='".$username."' ) and password='".$password."'");
                     if($pincode_chk->num_rows()==0)
                     {
                         return array('status' =>FALSE, 'message'=>"Invalid Pincode");
                     }
    ...
    function resendOTP($user_id)
    {
        ...
        if($chk->num_rows()>0)
        {
            ...
            if($this->send_message($otp_message,$phone))
            {
                $ar = array('otp'=>$otp);
                $wr = array('id'=>$user_id);
   ...
   function send_message($message = "", $mobile_number) {
    ...
    function checkForgot($phone,$type)
    {
            if($type=='doctor')
            {
                $chk = $this->db->query("select * from doctors where mobile_number='".$phone."' and doctor_show_status='active'");
                if($chk->num_rows()>0)
                {
                    $otp = rand(1000,10000);
                    //$otp = '1234';
                ...
                {
                    return array('status' =>FALSE, 'message'=>"Invalid Mobile Number");
                }
            }
            else
            {
    ...
    function resetPassword($phone,$otp,$password,$type)
    {
        if($type=='doctor')
        {
                $qry = $this->db->query("select * from doctors where mobile_number='".$phone."' and otp='".$otp."'");
            if($qry->num_rows()>0)
            {
                 ...
                 {
                    $row = $qry->row();
                    $res = array('status' =>TRUE,'user_id'=>$row->id,'phone'=>$row->mobile_number);
                    return $res;
                 }
            }
            else
    ...
    function getProfile($user_id)
    {
            ...
            if($qry->num_rows()>0)
            {
                ...
                $res = array('status' =>TRUE,'user_id'=>$row->id,'shop_name'=>$row->shop_name,'name'=>$row->owner_name,'email'=>$row->email,'description'=>$row->description,'address'=>$row->address,'city'=>$row->city,'pincode'=>$pincode,'phone'=>$row->mobile,'min_order_amount'=>$row->min_order_amount,'delivery_time'=>$row->delivery_time,'vendor_status'=>$vendor_status,'shop_image'=>$shopimg,'shop_logo'=>$shoplogo,'gender'=>$row->gender,'bio'=>$row->description,'youtube_id'=>$row->youtube_id);
                return $res;
            }
            else
            {
    ...
    {
        $qry = $this->db->query("select id,sub_category_name,cat_id from sub_categories where cat_id='".$cat_id."'");
        $dat = $qry->result();
        if($qry->num_rows()>0)
        {
            $ar=[];
...
function getSingleProduct($pid)
{
        ...
        if($qry->num_rows()>0)
        {
                   ...
                   $ar=array('id'=>$value->id,'product_title_telugu'=>$value->product_title_telugu,'shop_id'=>$value->shop_id,'name'=>$value->name,'description'=>$value->descp,'category_name'=>$category->category_name,'subcategory_name'=>$subcategory->sub_category_name,'brand'=>$value->brand,'brand_id'=>$value->brand,'shop'=>$vendor->shop_name,'product_tags'=>explode(",",$value->product_tags),'meta_tag_title'=>$value->meta_tag_title,'meta_tag_description'=>$value->meta_tag_description,'meta_tag_keywords'=>$value->meta_tag_keywords,'key_features'=>$value->key_features,'cancel_status'=>$value->cancel_status,'return_status'=>$value->return_status,'image'=>$img,'selling_date'=>date('d-m-Y',strtotime($value->selling_date)),'taxname'=>$value->taxname,'manage_stock'=>$value->manage_stock,'variant_product'=>$value->variant_product,'status'=>$value->status,'cat_id'=>$value->cat_id,'sub_cat_id'=>$value->sub_cat_id,'tax_class'=>$value->tax_class,'availabile_stock_status'=>$value->availabile_stock_status,'price'=>$price,'saleprice'=>$saleprice,'stock'=>$stock,'no_of_days'=>$value->return_noof_days,'exchangestatus'=>$exchangestatus,'product_type'=>$value->product_type);
    ...
    {
        $qry = $this->db->query("select * from products where cat_id='".$cat_id."'  and shop_id='".$shop_id."' and sub_cat_id='".$subcat_id."' and delete_status=0");
        $dat = $qry->result();
        if($qry->num_rows()>0)
        {   
                $ar=[];
...
/*
                   ...
                   $ar[]=array('id'=>$value->id,'shop_id'=>$value->shop_id,'variant_product'=>$value->variant_product,'name'=>$value->name,'product_title_telugu'=>$value->product_title_telugu,'description'=>$value->descp,'category_name'=>$category->category_name,'subcategory_name'=>$subcategory->sub_category_name,'brand'=>$value->brand,'shop'=>$vendor->shop_name,'product_tags'=>$value->product_tags,'meta_tag_title'=>$value->meta_tag_title,'meta_tag_description'=>$value->meta_tag_description,'meta_tag_keywords'=>$value->meta_tag_keywords,'key_features'=>$value->key_features,'cancel_status'=>$value->cancel_status,'return_status'=>$value->return_status,'image'=>$img,'availabile_stock_status'=>$value->availabile_stock_status);
                }
   ...
   function updateProduct($pid,$ar,$variant_json_data)
   {
           ...
           if($insert_query)
           {
               $novariant['product_id']=$pid;
                    ...
                    $name= $ar['name'];
                    $seo_url1 = preg_replace('/[^a-z0-9_-]/i', '', strtolower(str_replace(' ', '-', trim($name))));
                      $seo_url = $this->db->insert_id()."_".$seo_url1;
                    $this->db->update('products', array('seo_url'=>$seo_url), array('id'=>$pid));

                $jsondata = json_decode($variant_json_data);
                foreach ($jsondata as $value) 
                {

                   $json = array('attribute_type'=>$value->attribute_type,'attribute_value'=>$value->attribute_values);   
                   $jsondata = json_encode($json);
                   $variant_num_rows = $this->db->where(array('id'=>$value->id))->get('link_variant')->num_rows();
                   if($variant_num_rows>0)
                   {
                            $array = array(
                                //'product_id'=>$pid,
                                'price'=>$value->real_price,
                                'saleprice'=>$value->offer_price,
                                'jsondata'=>$jsondata,
                                'status'=>1
                            );
                            $where = array('id'=>$value->id);
                            $ins = $this->db->update("link_variant",$array,$where);
                   }
                   else
                   {
                            $array = array(
                            ...
                            $ins = $this->db->insert("link_variant",$array);
                           
                   }

                   if($value->isDelete==true || $value->isDelete=='true')
                   {
                        $where = array('id'=>$value->id);
                ...
                return array('status' =>TRUE,'product_id'=>$pid,'message'=>"Product updated successfully");
           }
           else
           {
                return array('status' =>FALSE, 'message'=>"Something went wrong");
   ...
   function insertProduct($ar,$imagepath,$variant_json_data)
   {
             
        $ar['cat_id']= $this->db->where(array('shop_id'=>$ar['shop_id']))->get('admin_comissions')->row()->cat_id;
       ...
       // echo $this->db->last_query(); die;
           ...
           if($insert_query)
           {
               $novariant['product_id']=$this->db->insert_id();
                    ...
                    $seo_url1 = preg_replace('/[^a-z0-9_-]/i', '', strtolower(str_replace(' ', '-', trim($name))));
                      $seo_url = $this->db->insert_id()."_".$seo_url1;
                   $this->db->update('products', array('seo_url'=>$seo_url), array('id'=>$novariant['product_id']));
               ...
               { 
                   ...
                   for ($i=0; $i < count($ex); $i++) 
                   { 
                        $this->db->insert('product_images', array('product_id'=>$novariant['product_id'],'image'=>$ex[$i]));
                   }
               }
                ...
                foreach ($jsondata as $value) 
                {
                   ...
                   $jsondata = json_encode($json);
                   $array = array(
                                'product_id'=>$novariant['product_id'],
                                'price'=>$value->real_price,
                                'saleprice'=>$value->offer_price,
                                'jsondata'=>$jsondata,
                                'status'=>1
   ...
   function novariant_insertProduct($ar)
   {
       ...
       if($insert_query)
       {
           $last_id=$this->db->insert_id();
            ...
            $name= $ar['name'];
                    $seo_url1 = preg_replace('/[^a-z0-9_-]/i', '', strtolower(str_replace(' ', '-', trim($name))));
                      $seo_url = $last_id."_".$seo_url1;
                   $this->db->update('products', array('seo_url'=>$seo_url), array('id'=>$last_id));
   ...
   {
       $qry = $this->db->query("select * from products where id='".$product_id."' and delete_status=0");
       $product = $qry->row();
   ...
   function browse_file($product_id)
    {
        $image = $this->upload_file('image');
        if($image!='false')
        {
                          $ins = $this->db->insert("product_images",array('product_id'=>$product_id,'image'=>$image));
                          
                          if($ins)
                          {
                            $image_id=$this->db->insert_id();
                              return array("image"=>$image,"image_id"=>$image_id);
                          }
                          else
                          {
                              return 'false';
    ...
    {
        $image = $this->uploadProducts('image');
        $fullpath = base_url()."uploads/products/".$image;
         return array('status'=>TRUE,'image'=>$image,'fullpath'=>$fullpath);  
        ...
        if($qry->num_rows()>0)
        {
                $var=[];
                    foreach($details as $dat)
                    {
                        ...
                        $qry = $this->db->query("select * from product_images where product_id='".$product_id."'");
                        $imagescount = $qry->num_rows();
                        
                          

                        $var[]=array('variant_id'=>$dat->id,'product_id'=>$dat->product_id,'price'=>$dat->price,'saleprice'=>$dat->saleprice,'stock'=>$dat->stock,'attributes'=>$attributelist,'image_count'=>$imagescount,'status'=>$dat->status);
                    }
   ...
   function getstockManagement($product_id,$variant_id)
   {
       ...
       if($qry->num_rows()>0)
       {
           ...
           {
               $prod = $this->db->query("select * from products where id='".$value->product_id."' and delete_status=0");
               $prodoct = $prod->row();
   ...
   function getAddVariantList($product_id)
   {
      ...
      if($qry->num_rows()>0)
      {
        ...
        {
            $pro = $this->db->query("select * from products where id='".$value->product_id."' and delete_status=0");
            $product = $pro->row();
   ...
   function updateVariant($pid,$attribute_type_id,$attribute_values,$vid)
    ...
    function getLinkVariants($pid)
    {
        ...
        if($mang->num_rows()>0)
        {
            foreach ($varint as $value) 
            {
                    ...
                    $images = $this->db->query("select * from product_images where product_id='".$pid."'");
                    ...
                    $ar[]=array('id'=>$value->id,'status'=>$value->status,'statusname'=>$statusname,'product_id'=>$value->product_id,'price'=>$price,'saleprice'=>$saleprice,'stock'=>$stock,'images_count'=>$images_count,'attributes'=>$attributes);
            }
    ...
    function fetchOrdersList($vendor_id)
    {
        ...
        if($qry->num_rows()>0)
        {
            ...
            {   
                ...
                $total_price = strval($value->total_price);
               $ar[]=array('id'=>$value->id,'session_id'=>$value->session_id,'customer_name'=>$name,'vendor_name'=>$vendor->shop_name,'address'=>$full_address,'payment_status'=>$payment_status,'payment_type'=>$value->payment_option,'service_status'=>$order_status,'amount'=>$total_price,'created_date'=>date('d-m-Y',$value->created_at));
            }
    ...
    function fetchProcessingOrdersList($vendor_id)
    {
        ...
        $qry = $this->db->query("select * from orders where vendor_id='".$vendor_id."' and order_status NOT IN (5,6) order by id desc");
        if($qry->num_rows()>0)
        {
            $result = $qry->result();
            $ar=[];
            ...
            {   
                ...
                {
                    $order_status = "packing_completed";
                }
                else if($value->order_status==5)
                {
                    $order_status = "Delivered";
               ...
               $ar[]=array('id'=>$value->id,'session_id'=>$value->session_id,'customer_name'=>$name,'vendor_name'=>$vendor->shop_name,'address'=>$full_address,'payment_status'=>$payment_status,'payment_type'=>$value->payment_option,'service_status'=>$order_status,'amount'=>$total_price,'created_date'=>date('d-m-Y',$value->created_at),'del_status'=>$del_status,'accept_status'=>$accept_status);
            }
            return array('status' =>TRUE, 'orders'=>$ar);
        }
        else
    ...
    {
        $qry = $this->db->query("select * from orders where vendor_id='".$vendor_id."' and order_status=5 order by id desc");
        if($qry->num_rows()>0)
        {
            $result = $qry->result();
            $ar=[];
     ...
     function getCancelledOrdersList($vendor_id)
    {
        $qry = $this->db->query("select * from orders where vendor_id='".$vendor_id."' and order_status=6 order by id desc");
        if($qry->num_rows()>0)
        {
            $result = $qry->result();
            $ar=[];
            ...
            {   
                ...
                $total_price = strval($value->total_price);
               $ar[]=array('id'=>$value->id,'session_id'=>$value->session_id,'reason'=>$value->reason,'customer_name'=>$name,'vendor_name'=>$vendor->shop_name,'address'=>$full_address,'payment_status'=>$payment_status,'payment_type'=>$value->payment_option,'service_status'=>$order_status,'amount'=>$total_price,'created_date'=>date('d-m-Y',$value->created_at));
            }
            return array('status' =>TRUE, 'orders'=>$ar);
        }
        else
    ...
    {
        $qry = $this->db->query("select * from orders where id='".$oid."' and vendor_id='".$vendor_id."'");
        if($qry->num_rows()>0)
        {
                            $value = $qry->row();
                            $cart = $this->db->query("select * from cart where session_id='".$value->session_id."' and vendor_id='".$vendor_id."'");
                            ...
                            foreach ($cartdetails as $c) 
                            {
                                    ...
                                    $pro_img = $this->db->query("select * from product_images where product_id='".$variants->product_id."'");
                        ...
                        $types = $type->row();
                            ...
                            $attributes=array('title'=>$types->title,'value'=>$attrvalue);
                                ...
                                $cartdata[]=array('cartid'=>$c->id,'image'=>$image,'productname'=>$products_name,'price'=>$c->price,'quantity'=>$c->quantity,'unit_price'=>$c->unit_price,'attributes'=>$attributes);
                            }
                ...
                if($action_person==0)
                {
                    $cancelled_by_whom = "User";
                }
                else if($action_person==1)
                {
                    $cancelled_by_whom = "You";
                }
                $coupon_discount=$value->coupon_disount;
               $ar=array('id'=>$value->id,'session_id'=>$value->session_id,'delivery_date'=>$value->delivery_timeslots,'gst'=>$value->gst,'order_status'=>$order_status,'vendor_name'=>$vendor->shop_name,'address'=>$value->user_address,'payment_status'=>$payment_status,'payment_type'=>$value->payment_option,'amount'=>$amount,'sub_total'=>$value->sub_total,'placed_on'=>date('d-m-Y',$value->created_at),'cartdetails'=>$cartdata,'customer_name'=>$name,'mobile'=>$address->mobile,'coupon_disount'=>$coupon_discount,'deliveryboy_commission'=>$value->deliveryboy_commission,'delivery_name'=>$dl_name,'delivery_phone'=>$dl_phone,'alternative_mobiles'=>$alternative_mobiles,'order_status'=>$show,'accept_status'=>$accept_status,'bidstatus'=>$bidstatus,'reason'=>$value->reason,'comments'=>$value->comments,'cancelled_by_whom'=>$cancelled_by_whom);
    ...
    {   
        $cdate = date('Y-m-d');
       $ongoing_qry = $this->db->query("select * from orders where vendor_id='".$vendor_id."' and created_date LIKE '".$cdate."%'");
       $ongoing_count = $ongoing_qry->num_rows();


       $completed_qry = $this->db->query("select * from orders where vendor_id='".$vendor_id."' and created_date LIKE '".$cdate."%' and order_status=5");
       $completed_row = $completed_qry->num_rows();


       $pending = $ongoing_count-$completed_row;

        
        $new_order_qry = $this->db->query("select * from orders where vendor_id='".$vendor_id."' and order_status=1");
       $new_order_count = $new_order_qry->num_rows();

        return array('status' =>TRUE, 'ongoing_count'=>$ongoing_count,'completed_count'=>$completed_row,'pending'=>$pending,'notification_count'=>$new_order_count );
    }
...
function doacceptOrder($vendor_id,$orderid)
    {
        ...
        {
            return array('status' =>FALSE,'message'=>"This Order already cancelled");
        }
                    ...
                    $vendor_row = $vendor_qry->row();
                        ...
                        $message = "Your Order Accepted";
                            ...
                            $this->onesignalnotification($user_id,$message,$title);
                                            ...
                                            $user_order_message = "Dear ".$user_name." your order is ".$session_id." is accepted by the vendor and it will be delivered with in 2-24hrs of time. Thank u for shopping with A3 Services.";
                                           
                                            $this->send_message($user_order_message,$user_phone);

                                            $user_order_message1 = "Order Accepted";
                                               $this->db->insert("sms_notifications",array('order_id'=>$session_id,'receiver_id'=>$user_id,'sender_id'=>$vendor_id,'created_at'=>time(),'message'=>$user_order_message1,'action_person'=>1,'order_status'=>$user_order_message1,'status'=>'order_accepted')); 
    ...
    function docancelOrder($vendor_id,$orderid,$reason)
    {
        $qry = $this->db->query("select * from orders where id='".$orderid."'");
        $row = $qry->row();
...
$d1 = new DateTime($row->created_date);
...
if($diffInMinutes>20)
{
 return array('status' =>FALSE,'message'=>"Dear vendor You cannot cancel the order as the time is reached to 20 minutes");
}


            $ar = array('order_status'=>'6','reason'=>$reason);
            $wr = array('id'=>$orderid);
            $upd = $this->db->update("orders",$ar,$wr);
            if($upd)
            {
                ...
                $msg1="Dear".$name." your order no. ".$row->session_id." is cancelled";

                $phone = $user_row->phone;
                $this->send_message($msg1,$phone);

                $order_status = 'Order Cancelled by vendor';
                $this->db->insert("sms_notifications",array('order_id'=>$orderid,'receiver_id'=>$user_id,'sender_id'=>$vendor_id,'created_at'=>time(),'message'=>$msg1,'action_person'=>1,'order_status'=>$order_status,'status'=>'order_cancel')); 
    ...
    function updateProfileDetails($vendor_id,$shop_name,$owner_name,$gender,$mobile,$description,$address,$delivery_charges,$youtube_url)
    {
        if($description!='')
        {
           $des = $description;
        ...
        $ar = array("shop_name"=>$shop_name,"owner_name"=>$owner_name,"description"=>$des,"address"=>$adrs,"mobile"=>$altr_mobile,'min_order_amount'=>$deliverycharge,'gender'=>$gender,'youtube_id'=>$youtube_url,'min_order_amount'=>$delivery_charges);
        $wr = array("id"=>$vendor_id);
        $upd = $this->db->update("vendor_shop",$ar,$wr);
    ...
    {
        $del = $this->db->update("products",array('delete_status'=>1),array('id'=>$pid));
        if($del)
        {
           /* $this->db->delete("product_images",array('product_id'=>$pid));
        ...
        /*$check_qry = $this->db->query("SELECT * FROM `link_variant` WHERE product_id='".$pid."'");
        $check_row = */
        ...
        if($qry->num_rows()>0)
        {
            ...
            foreach ($result as $rep) 
            {
                ...
                else if($value->order_status==7)
                {
                    ...
                    $pro = $this->db->query("select * from products where id='".$rep->product_id."' and delete_status=0");
                    $product = $pro->row();
 ...
 function getproductsFilters($json_data,$shop_id,$cat_id)
{
     ...
     $str = json_encode($json_data);
        ...
        {   
                ...
                {
                    $qry11 = $this->db->query("select * from products where cat_id='".$cat_id."'  and shop_id='".$shop_id."' and id='".$value12->product_id."' and delete_status=0");
                    $value = $qry11->row();
...
function getUsersBids($vendor_id,$bid_status)
{
    ...
    {
     $qry = $this->db->query("SELECT user_bids.*,vendor_bids.created_date,vendor_bids.admin_commission FROM user_bids INNER JOIN vendor_bids ON vendor_bids.bid_id=user_bids.id where vendor_bids.vendor_id='".$vendor_id."' and user_bids.bid_status=0 order by user_bids.id desc");
        
    }
    else if($bid_status=='accepted')
    {
        $qry = $this->db->query("SELECT user_bids.*,vendor_bids.created_date,vendor_bids.admin_commission FROM user_bids INNER JOIN vendor_bids ON vendor_bids.bid_id=user_bids.id where vendor_bids.vendor_id='".$vendor_id."' and user_bids.bid_status=1 order by user_bids.id desc");
    ...
    {
        $qry = $this->db->query("SELECT user_bids.*,vendor_bids.created_date,vendor_bids.admin_commission FROM user_bids INNER JOIN vendor_bids ON vendor_bids.bid_id=user_bids.id where vendor_bids.vendor_id='".$vendor_id."' and user_bids.bid_status=2 order by user_bids.id desc");
    ...
    if($qry->num_rows()>0)
    {
        ...
        foreach ($cart_row as $value) 
        {
            ...
            $pro_img = $this->db->query("select * from product_images where product_id='".$variant_row->product_id."'");
            $pro_imgs = $pro_img->row();
...
function deleteBanner($cid)
{
    ...
    if($del)
    {
        return array('status' =>TRUE,'message'=>"Banner deleted Successfully");
    }
}
...
function searchPreLoadedProducts($keyword,$shopId)
{
    ...
    $im = implode(",", $catis);
    $prod = $this->db->query("select id,name from products where find_in_set(cat_id,'".$im."') and name LIKE '%".$keyword."%' and delete_status=0");
    return $prod->result();
}
...
{
    $upd = $this->db->update("vendor_shop",array('token'=>''),array('id'=>$user_id));
    if($upd)
    {
...
function homepageSalesReport($vendor_id,$start_date=null,$end_date=null)
{
    if($start_date!='' && $end_date!='')
    {
       $qry = $this->db->query("select * from orders where vendor_id='".$vendor_id."' and order_status=5 and order_date BETWEEN '".$start_date."' AND '".$end_date."' order by id desc");
    }
    else
    {
        $current= date('Y-m-d');
        $qry = $this->db->query("select * from orders where vendor_id='".$vendor_id."' and order_status=5 and created_date LIKE '".$current."%' order by id desc");
    }
        ...
        if($qry->num_rows()>0)
        {
            ...
            {   
               ...
               $ar[]=array('id'=>$value->id,'session_id'=>$value->session_id,'payment_type'=>$value->payment_option,'order_status'=>$order_status,'amount'=>$value->total_price,'created_date'=>date('d-m-Y',$value->created_at));

               $total_vendor_commission=$total_vendor_commission+$value->vendor_commission;
            }


            $current= date('Y-m-d');
           $today_vendor_qry = $this->db->query("select SUM(vendor_commission) as today_vendor_commission from orders where vendor_id='".$vendor_id."' and order_status=5 and created_date LIKE '".$current."%' order by id desc");
           $today_vendor_row = $today_vendor_qry->row();


           $total_vendor_qry = $this->db->query("select SUM(vendor_commission) as total_vendor_commission from orders where vendor_id='".$vendor_id."' and order_status=5 order by id desc");
           $total_vendor_row = $total_vendor_qry->row();

            $sales =array('today_vendor_commission'=>number_format($today_vendor_row->today_vendor_commission,2),'total_vendor_commission'=>number_format($total_vendor_commission,2),'vendor_monthly_commission'=>"");

            return array('status' =>TRUE, 'orders'=>$ar,'sales_report'=>$sales);
        }
        else
        {
            return array('status' =>FALSE, 'message'=>"No Orders");
...
function getAttributes($shop_id)
{
    $vendor_rows = $this->db->where(array('id'=>$shop_id))->get('vendor_shop')->num_rows();
    if($vendor_rows>0)
    {
              $this->db->select("id,title");
        $data=$this->db->get('attributes_title')->result();
        return array('status' =>TRUE, 'data'=>$data);
    }
    else
    {
        return array('status' =>FALSE, 'message'=>"Invalid Vendor Details");
    }
}

function getShopAttributeValues($shop_id,$attribute_id)
{
    $vendor_rows = $this->db->where(array('id'=>$shop_id))->get('vendor_shop')->num_rows();
    if($vendor_rows>0)
    {
              $this->db->select("id,value");
        $data = $this->db->where(array('attribute_titleid'=>$attribute_id))->get('attributes_values')->result();
...
function getShopWiseSubCategories($shop_id)
{
        $qry = $this->db->where(array('shop_id'=>$shop_id))->get('admin_comissions');
        $value = $qry->row();
        if($qry->num_rows()>0)
        {   
                $subcategory = $this->db->where(array('cat_id'=>$value->cat_id))->get('sub_categories')->result();
                $ar=[];
                if(count($subcategory)>0)
                {
                    foreach ($subcategory as $category) {
                        ...
                        if($category->app_image!='')
                        {
                            $img = base_url()."uploads/sub_categories/".$category->app_image;
                        }
                        else
                        {
                            $img = "";
                       ...
                       $ar[]=array('id'=>$category->id,'title'=>$category->sub_category_name,'image'=>$img);
                    }
        ...
        {
            return array('status' =>FALSE, 'message'=>"Categories Not Assigned");
        }
}


function uploadImage($shop_id)
    {
        return $this->upload_file('image');
    }

function getProductDetails($product_id)
{
    $product_details = $this->db->where(array('id'=>$product_id))->get('products')->row();

    $num_rows = $this->db->where(array('id'=>$product_id))->get('products')->num_rows();
        $this->db->select("id,image");
    $product_images = $this->db->where(array('product_id'=>$product_id))->get('product_images')->result();
    foreach ($product_images as $value) 
    {
       $value->image=base_url()."uploads/products/".$value->image;
    }

    $product_details->product_images = $product_images;
                    $this->db->select("id,price,saleprice,jsondata");
    $link_variants = $this->db->where(array('product_id'=>$product_id))->get('link_variant')->result();
    foreach ($link_variants as $link_value) 
    {
        $link_value->jsondata= json_decode($link_value->jsondata);
    }
    $product_details->link_variants = $link_variants;
    if($num_rows>0)
    {
        return array('status' =>TRUE,'data'=>$product_details);
    }
    else
    {
        return array('status' =>FALSE,'message'=>"Invalid product");
    }
    
}


function packing_complete($vendor_id,$id)
{
        $data=array('order_status'=>3);
        $this->db->where('id',$id,'vendor_id',$vendor_id);
        $update =  $this->db->update('orders',$data);
       //echo $this->db->last_query(); die;
        if($update)
        {
            $order_row=$this->db->where(array('id'=>$id))->get('orders')->row();
            $session_id = $order_row->session_id;
            $user_id = $order_row->user_id;
            $vendor_id = $order_row->vendor_id;

            $user_order_message1 = "Packing Completed";
             $this->db->insert("sms_notifications",array('order_id'=>$session_id,'receiver_id'=>$user_id,'sender_id'=>$vendor_id,'created_at'=>time(),'message'=>$user_order_message1,'action_person'=>1,'order_status'=>$user_order_message1,'status'=>'packing_completed')); 

                $user_row = $this->db->where(array('id'=>$user_id))->get('users')->row();

                $user_name = $user_row->first_name;
                $user_phone = $user_row->phone;
            $user_order_message = "Dear ".$user_name." our Delivery person is on the way with your order ".$session_id;

                 return array('status' =>TRUE,'message'=>"Packing Completed Successfully");
        }
        else
        {

         return array('status' =>TRUE,'message'=>"Something went wrong, please try again");
        }
}
