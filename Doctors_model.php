...
function notificationCount($user_id)
{
    ...
    function symptomWiseDoctors($symptom_id)
    {
        ...
        $doctor_ids = implode(",", $doct_id);
            ...
            foreach ($data as $value) {
                ...
                $doctor_image = base_url()."uploads/doctors/".$value->doctor_image;
                $designations = $this->get_designation_names_csv($designations);
                 $array[] = array('id'=>$value->id,'hospital_name'=>$hospital_name,'hospital_image'=>$hospital_image,'doctor_name'=>$doctor_name,'doctor_image'=>$doctor_image,'designations'=>$designations,'consultant_fee'=>$value->consultant_fee,'blue_tick'=>$blue_tick,'doctor_rating' =>$doctor_rating,'total_users_reviewed' =>  $total_users_reviewed);
    ...
    function symptomSubsymptomWiseDoctors($symptom_id,$subsymptom_id=null)
    {
        ...
        // print_R($value);die; 
              ...
              $doctor_reviews = $value->doctor_reviews;
...
function updateNotifications($user_id)
{
    ...
    function initiatePhonePeSubscriptionPayment($doctor_id, $plan_id) {
        ...
        // Load PhonePe SDK Client (Using working credentials from appointment booking)
        ...
        // Use PhonePe Official SDK for payment initiation
        ...
        $payment_result = $this->phonepesdkclient->initiatePayment(
        ...
        // Update payment record with PhonePe details
        $this->db->where('id', $payment_id);
        $this->db->update('doctor_subscription_payments', array(
            'transaction_id' => $merchant_transaction_id,
            'phonepe_order_id' => $payment_result['data']['orderId'],
            'phonepe_redirect_url' => $payment_result['data']['redirectUrl']
        ));

        // Return PhonePe SDK response
        return array(
            'status' => TRUE,
            ...
            'phonepe_config' => array(
                'redirectUrl' => $payment_result['data']['redirectUrl'],
                'merchantOrderId' => $payment_result['data']['merchantOrderId'],
                'orderId' => $payment_result['data']['orderId'],
                'status' => $payment_result['data']['status'],
                'expiresAt' => $payment_result['data']['expiresAt'],
                // Additional fields for Flutter SDK
    ...
    function verifyPhonePePayment($merchant_transaction_id) {
        // Load PhonePe SDK Client (Using working credentials from appointment booking)
        $this->load->library('PhonePeSDKClient');
        
        // Verify payment using PhonePe SDK
        $verification_result = $this->phonepesdkclient->getOrderStatus($merchant_transaction_id, true);
        ...
        $order_data = $verification_result['data'];
        
        // Update payment status based on PhonePe response
        $payment_status = 'failed';
        if ($order_data['status'] === 'PAYMENT_SUCCESS') {
            $payment_status = 'success';
        } elseif ($order_data['status'] === 'PAYMENT_PENDING') {
            $payment_status = 'pending';
        }
        ...
        $this->db->update('doctor_subscription_payments', array(
            ...
            'phonepe_response' => json_encode($order_data),
